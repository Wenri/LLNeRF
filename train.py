    ...
    for step, batch in zip(range(init_step, num_steps + 1), pdataset):
        ...
        if jax.host_id() == 0:
            ...
            if step == init_step or step % config.print_every == 0:
                ...
                print_func(
                    f'[{datetime.datetime.now().strftime("%H:%M:%S")}] {step:{precision}d}' + f'/{config.max_steps:d}: ' +
            ...
            if step == 1 or step % config.checkpoint_every == 0:
                ...
                if step == 1:
                    ...
                    shutil.rmtree(os.path.join(config.checkpoint_dir, 'checkpoint_1'))
        ...
        if valid_condition:
            ...
            if jax.host_id() == 0:
                ...
                if config.vis_decimate > 1:
                    decimate_fn = lambda x, d=config.vis_decimate: None if x is None else x[::d, ::d]
                else:
